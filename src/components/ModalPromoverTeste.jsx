...
export default function ModalPromoverTeste({ projeto, onClose, onPromoted }) {
  ...
  const lbl = { fontSize: 11, fontWeight: 600, color: '#3D4E62', textTransform: 'uppercase', letterSpacing: 0.5 }
    ...
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,32,62,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999 }}>
      <div onClick={e => e.stopPropagation()} style={card}>
        ...
        <div style={{ padding: 22 }}>
          {!resumo ? (
            <div style={{ color: '#3D4E62', fontSize: 13, textAlign: 'center', padding: 20 }}>Carregando diagnóstico…</div>
          ) : resumo.pendentes > 0 ? (
            ...
            <>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 18 }}>
                <div style={{ background: 'rgba(34,197,94,0.08)', borderRadius: 8, padding: '10px 12px' }}>
                  ...
                  <div style={{ fontSize: 10, color: '#3D4E62' }}>vão para teste</div>
                </div>
                ...
                <div style={{ background: 'rgba(239,68,68,0.08)', borderRadius: 8, padding: '10px 12px' }}>
                  ...
                  <div style={{ fontSize: 10, color: '#3D4E62' }}>viram GAP (N1)</div>
                </div>
              </div>

              <div style={{ marginBottom: 16 }}>
                <div style={lbl}>Até qual fase o projeto vai? <span style={{ color: '#3D4E62', textTransform: 'none', letterSpacing: 0, fontWeight: 400 }}>(distribui os %)</span></div>
                <select value={numFases} onChange={e => setNumFases(parseInt(e.target.value))} style={{ width: '100%', maxWidth: 320, marginTop: 6, padding: '9px 12px', border: '1px solid #D0D0D0', borderRadius: 6, fontFamily: 'inherit', fontSize: 13 }}>
                  {[1, 2, 3, 4, 5].map(n => <option key={n} value={n}>{n === 1 ? 'Até Fase 1 - Diagnóstico Inicial Com Testes' : FASES_LABEL[n]}</option>)}
                </select>
                <div style={{ fontSize: 11, color: '#3D4E62', marginTop: 6 }}>{numFases === 1 ? 'Projeto vai até a Fase 1 — agora com o teste de efetividade do controle (régua N1–N5). O diagnóstico já feito é o ponto de partida.' : FASES_DETALHE[numFases]}</div>
              </div>
